...
1. **Backend**
   ...
   **Manual alternative**
   ...
   - **Fallback**: if you use the autogenerated self-signed cert (now issued with SAN entries for `localhost`, `127.0.0.1`, and `::1`), open `~/.finalextension/localhost-cert.pem` in Keychain Access, set “When using this certificate” to “Always Trust”, and restart Chrome. You can disable TLS entirely for local debugging via `SENTINEL_HTTP_ONLY=1 python app.py`, which serves plain HTTP on `http://localhost:5000`.
